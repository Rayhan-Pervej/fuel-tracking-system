...
class TransactionService:
    ...
    @staticmethod
    def build_query(from_date=None, to_date=None, vehicle_id=None, pump_id=None, vehicle_number=None, pump_name=None, pump_license=None, fuel_type=None):
        query = {}
        if vehicle_id:
        ...
        if vehicle_number:
            vehicle_ids = VehicleModel.get_ids_by_number(vehicle_number)
            query["vehicle_id"] = {"$in": vehicle_ids}
        if pump_name:
            pump_ids = PumpModel.get_ids_by_name(pump_name)
            query["pump_id"] = {"$in": pump_ids}
        if pump_license:
            pump_id_from_license = PumpModel.get_id_by_license(pump_license)
            if pump_id_from_license:
                query["pump_id"] = pump_id_from_license
        if fuel_type:
    ...
    @staticmethod
    def get_filtered(from_date, to_date, vehicle_id=None, pump_id=None, vehicle_number=None, pump_name=None, pump_license=None, fuel_type=None, cursor=None, limit=10):
        query = TransactionService.build_query(from_date, to_date, vehicle_id, pump_id, vehicle_number, pump_name, pump_license, fuel_type)
